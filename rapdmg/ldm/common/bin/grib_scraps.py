...
if __name__ == "__main__":
  ...
  for root, dirs, files in walk(tmp_dir):
    ...
    for name in files:
      ...
      if find(tmp_grib_path, 'grb2') != -1 and find(tmp_grib_path, '.gz') == -1:
        status_file = replace(tmp_grib_path, 'grb2', 'status')

        status_size = find_status_size(status_file)

        if status_size <= 0:
          print "WARNING: status_size <=0 : ",status_size,"for",status_file
          continue
